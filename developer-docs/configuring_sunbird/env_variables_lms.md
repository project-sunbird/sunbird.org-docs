...
| 29 | sunbird_url_shortner_enable | Configuration to enable generation of shortened URLs. Bitly is used to generate the short URLs. | Email and SMS notifications can include short URLs. | false | Notification
| 30 | sunbird_url_shortner_access_token | Access token of Bitly account. | Email and SMS notifications can include short URLs. This is required if URL shortner is enabled. | | Notification
| 31 | sunbird_keycloak_required_action_link_expiration_seconds | Configuration for user required action (e.g. set password, verify email) link expiration (in seconds). | Set expiration time for set password and verify email required action link for new user. | 7200 | User, Notification
| 32 | sunbird_user_profile_field_default_visibility | Configuration for default visibility (public / private) of user profile fields. | | public | User
