...
| 29 | sunbird_allowed_login | Customise login message in welcome email and SMS template sent on new user creation. | Welcome notification to new Sunbird users | You can use your cellphone number to login | Learner-Service
| 30 | sunbird_course_batch_notification_signature | Enable or disable course batch notifications to added or removed participants and mentors on course batch create or update. | Course batch email notifications | sunbird | Learner-Service 
| 31 | sunbird_course_batch_notification_enabled | Customise email signature in the notification sent when a participant or mentor is added or removed from a course batch. | Course batch email notifications | false | Learner-Service 
| 32 | sunbird_authorization | API key configuration in Keycloak for invoking Sunbird platform APIs (e.g. Email notification API) | Email notification (e.g. forgot password email with OTP) | Keycloak-Service 
| 33 | sunbird_lms_base_url | Base URL (e.g. https://staging.open-sunbird.org/api) configuration for invoking Sunbird platform APIs (e.g. email notification API). | Email notification (e.g. forgot password email with OTP) | | Keycloak-Service
