...
published: true
allowSearch: true
keywords: Upgrade, Sunbird 2.5.0, etl jobs
---
...
As part of release-2.5.0, we were encrypting all user external identity in cassandra, but since this is not private data
from 2.5.0 we are storing it as plain text. For data which is already encrypted, below is the jenkins job to decrypt the 
data.
