...
> Note: The path has the following format <base_url>/content/v1/upload/<content_id>. For example, the path using the sample value in the **Path Parameters** section for the content ID parameter in the sunbird staging environment will be: https://staging.open-sunbird.org/api/content/v1/upload/do_21258058227713638416 
...
For YouTube or other content found on the web, you can upload the URL of the content artifact, pass it as a `fileUrl` in [Upload API](apis/content/#operation/Upload%20Content) or set the artifactUrl of the content using [Update Content API](apis/content/#operation/Update%20Content)
